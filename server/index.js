...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Allow same-origin requests (when frontend and backend are on same server)
    if (origin === `http://localhost:${process.env.PORT || 5001}` || 
        origin === `http://127.0.0.1:${process.env.PORT || 5001}`) {
      return callback(null, true);
    }
...
// Static file serving with performance optimizations
app.use('/uploads', express.static(path.join(__dirname, '../uploads'), {
  maxAge: '30d', // Increased from 1d to 30d for portfolio images
  etag: true,
  lastModified: true
}));
...
if (process.env.NODE_ENV === 'production') {
  app.use(express.static(path.join(__dirname, '../client/build'), {
    maxAge: '1d', // Cache HTML files for 1 day
    etag: true,
    lastModified: true,
    setHeaders: (res, path) => {
      ...
      } else if (path.endsWith('.svg') || path.endsWith('.png') || path.endsWith('.jpg') || path.endsWith('.jpeg') || path.endsWith('.webp')) {
