...
import { softwareLogos } from '../../services/technologyData';
...
const Header: React.FC = () => {
  ...
  const allServices = Object.values(services).flatMap(category => category.items.map(item => ({
    type: 'service',
    name: item.name,
    desc: item.desc,
    href: item.href,
  ...
  const allTechnologies = Object.entries(softwareLogos).flatMap(([serviceSlug, techs]) => 
    Object.keys(techs).map(techName => {
      const service = Object.values(services).flatMap(cat => cat.items).find(item => item.href === `/services/${serviceSlug}`);
      return {
        type: 'technology',
        name: techName,
      ...
      <div className="sticky top-0 z-50 p-4">
        <div className="flex items-center justify-center space-x-4 max-w-screen-2xl mx-auto">
          ...
          {isAuthenticated ? (
            ...
            <Link
              to="/login"
              className="hidden xl:inline-flex items-center justify-center px-6 py-3 border border-transparent text-sm font-semibold rounded-full text-white bg-[#F39C12] hover:bg-[#E67E22] transition-colors duration-300 flex-shrink-0 shadow-lg"
            >
              Login
            </Link>
          )}
            ...
            <div className="container mx-auto px-4">
              <div className="relative flex items-center justify-between h-20">
                ...
                <div className="flex items-center justify-start xl:w-1/4">
                  <button onClick={handleSidebarToggle} className="p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                    <LayoutGrid size={22} />
                  </button>
                  <button
                    className="hidden xl:block p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
                ...
                <div className="flex items-center justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                                  ...
                                  <div key={category.name} className="relative group">
                                    ...
                                    >
                                      <h3 className={`font-bold text-base text-gray-900 dark:text-white mb-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors flex items-center ${isLeftColumn ? 'justify-end' : ''}`}>
                                        {isLeftColumn ? ( <> {category.name} <category.icon className="h-5 w-5 ml-2 text-gray-400 group-hover:text-cyan-500 transition-colors" /> </>
                                        ) : ( <> <category.icon className="h-5 w-5 mr-3 text-gray-400 group-hover:text-cyan-500 transition-colors" /> {category.name} </>
                                        )}
                                      </h3>
                  ...
                  <div className="px-6 flex-shrink-0">
                    <Link to="/">
                      <img src={theme === 'light' ? "/logo.svg" : "/logo-dark.svg"} alt="ClickBit Logo" className="h-8" />
                    </Link>
                  </div>
                  <nav className="hidden xl:flex items-center space-x-2">
                    <NavLink 
                ...
                <div className="flex items-center justify-end xl:w-1/4">
                  ...
                  <div className="hidden xl:flex items-center space-x-2">
                    <Link to="/cart" className="relative p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                      <ShoppingCartIcon className="h-6 w-6" />
                      {itemCount > 0 && (
                        <span className="absolute top-0 right-0 block h-4 w-4 rounded-full bg-[#1FBBD2] text-white text-xs font-medium text-center transform -translate-y-1/2 translate-x-1/2">
                          {itemCount}
                        </span>
                      )}
                    </Link>
                ...
                <div className="mt-6">
                  <nav className="grid gap-y-2">
                    ...
                    <div>
                      ...
                      >
                        Services
                        <ChevronDownIcon className={`h-5 w-5 transition-transform ${isMobileServicesOpen ? 'rotate-180' : ''}`} />
                      </button>
                      <AnimatePresence>
                        {isMobileServicesOpen && (
                          <motion.div
                            ...
                            className="overflow-hidden pl-4 border-l-2 border-cyan-500/50 ml-4"
                          >
                            <div className="py-2">
                              {Object.values(services).map(category => (
                                <MobileNavLink key={category.name} to={`/services/${category.slug}`} onClick={closeMobileMenu}>
                                  {category.name}
                                </MobileNavLink>
                              ))}
                              <MobileNavLink to="/services" onClick={closeMobileMenu}>View All</MobileNavLink>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
